def popular_item(session, W_ID, D_ID, L):
    ...
    # compute the next available order number for the district table
    rows = session.execute("SELECT d_next_o_id FROM district WHERE D_W_ID = {} AND D_ID = {};".format(W_ID, D_ID))
    print(rows.one() )
    N = rows.one().d_next_o_id

    # compute the set of last L orders for the district table
    ...
    for order in last_orders:
        # for each order, compute the maximum ol_quantity
        ...
        # select items that have quantities equal to item_highest_quantity
